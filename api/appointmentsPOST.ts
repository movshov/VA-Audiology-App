...
import db from './db';
import { AppointmentJsonSubmission } from '../api-objects/AppointmentJsonSubmission';
...
export default handler(async (request: any, userId) => {
  ...
  let requestVars:AppointmentJsonSubmission =  new AppointmentJsonSubmission(request.body);
  ...
  // Appointments inserted last because it needs info from the other tables
    return await Promise.all([
      ...
      body['authorityid'] = 111; // userId//TODO: Need to rebase onto authentication to get 
