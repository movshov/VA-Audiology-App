...
export class AudiologistSummaryComponent implements OnInit {
   ...
   */
  constructor(public thsDataService: ThsDataService, public tsDataService: TsScreenerDataService, public tfiDataService: TfiDataService, public testsDataService: TestsDataService) {
  ...
  private sumTHS(array: Array<{ state: any; choice: any; }>, start: number, length: number): number {
    let sum: number = 0;
  ...
  private calcTFIsub(array: Array<{ state: number; choice: number; }>, start: number, length: number): number {
    let score: number = 0;
    if (array.length < 1) {
      return;
    }
